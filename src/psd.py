# -*- coding: utf-8 -*-
"""
Created on Thu Jul 30 16:34:13 2026
...
cálculo de FFT/PSD com filtro Passa-Banda e identificação dos 5 picos dominantes.
"""
...
def carregar_dados_teste(base_dir, teste):
    ...
    print(f"Offset automático calculated: {offset_tempo:.2f} s")
...
# =====================================================
# Cálculo da PSD (Densidade Espectral de Potência - Welch)
# =====================================================
def calcular_psd(sinal, fs, nperseg=2048):
    sinal = sinal - np.mean(sinal)
    nperseg = min(nperseg, len(sinal))
    # Alterado 'hanning' para 'hann'
    freqs, psd = welch(sinal, fs=fs, window='hann', nperseg=nperseg, scaling='density')
    return freqs, psd
...
# Calcular PSDs dos sinais filtrados
freqs_psd_acl, psd_acl_filt = calcular_psd(acl_filtrado, fs_comum)
freqs_psd_pzt, psd_pzt_filt = calcular_psd(pzt_filtrado, fs_comum)
...
# =====================================================
# PLOT 4: PSD (Power Spectral Density - Método Welch)
# =====================================================
fig4, ax4 = plt.subplots(figsize=(12, 6), dpi=150)

ax4.plot(freqs_psd_acl, psd_acl_filt, label="Accelerometer (PSD)", c='green', alpha=0.8)
ax4.plot(freqs_psd_pzt, psd_pzt_filt, label="Piezo (PSD)", c='black', alpha=0.8)

My_axis(ax4,
        xlim=[0, 100],
        ylim=[1e-6, max(np.max(psd_acl_filt), np.max(psd_pzt_filt)) * 2],
        logy=True,
        legbox=[0.98, 0.98, 2, 12],
        font=18,
        locator=True,
        setaxis=[f"Power Spectral Density (PSD - Welch)\n",
                 "Frequency (Hz)",
                 "PSD (Amplitude²/Hz)"])
