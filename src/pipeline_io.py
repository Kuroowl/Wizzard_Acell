...
def salvar_grupo(df: pd.DataFrame, sensor: str, condicao: str, saida_dir: Path,
    ...
    if downcast_float32:
        ...
        df[cols_float] = df[cols_float].astype("float32")# -*- coding: utf-8 -*-
"""
pipeline_io.py
Funções compartilhadas entre as etapas do pipeline para ler/escrever
dados particionados por sensor/condição (em vez de um único arquivo
...
def registrar_log(raiz_path: Path, etapa: str, parametros: dict) -> Path:
    """
    ...
    configuração cada figura/arquivo foi gerado.

    Arquivo: DadosTratados/Logs/pipeline_log.txt (append, nunca sobrescreve).
    """
